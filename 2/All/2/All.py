...
class RelogioBase(ABC):
    ...
    def seg_min(self):
        self.converter("segundos", "minutos", 60)

    def min_seg(self):
        self.converter("minutos", "segundos", 60)

    def seg_hora(self):
        self.converter("segundos", "horas", 3600)

    def hora_seg(self):
        self.converter("horas", "segundos", 3600)

    def dia_mes(self):
        self.converter("dias", "meses", 30)

    def mes_dia(self):
        self.converter("meses", "dias", 30)

    def mes_ano(self):
        self.converter("meses", "anos", 12)

    def ano_mes(self):
        self.converter("anos", "meses", 12)

    def ano_dia(self):
        self.converter("anos", "dias", 365)

    def dia_ano(self):
        self.converter("dias", "anos", 365)

    def ano_dec(self):
        self.converter("anos", "décadas", 10)

    def dec_ano(self):
        self.converter("décadas", "anos", 10)

    def dec_sec(self):
        self.converter("décadas", "séculos", 10)

    def sec_dec(self):
        self.converter("séculos", "décadas", 10)

    def sec_ano(self):
        self.converter("séculos", "anos", 100)

    def ano_sec(self):
        self.converter("anos", "séculos", 100)

    def converter(self, from_unit, to_unit, factor):
        value = input(f"Diga de quantos {from_unit} você quer transformar para {to_unit}: ")
        result = int(value) / factor if "min" in to_unit or "hora" in to_unit or "ano" in to_unit else int(value) * factor
        print(f"De {value} {from_unit} foram possíveis calcular {result} {to_unit}")
...
class Relogio(RelogioBase):        
    ...
    def conversoes(self):
        while True:
            ...
            elif con in self.conversoes_map:
                self.conversoes_map[con]()
